...
class DashboardModel extends Model
{
    ...
    {
        return $this->query("SELECT nasabah.periode, COUNT(nasabah.id_nsb) AS _total, YEAR(periode) AS _year, MONTH(periode) AS _month FROM `nasabah` WHERE 1 GROUP BY periode");
    }
    ...
    public function criteriaGraph(array $ids = [])
    {
        ...
        $sql = "SELECT * FROM `pengajuan` 
                    ...
                    WHERE pengajuan.id_nsb IN($inQuery) AND (kriteria.nama_kriteria = 'pekerjaan' OR kriteria.nama_kriteria = 'jangka waktu') AND sub_kriteria.ket != ''";
